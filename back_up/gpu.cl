...
void to_global_mem_array(__global double *buffer, double4 vector, 
        ...
        if(vector.y < 1.e-5){
            *(buffer+index+(*counter)*(*gsz)) = *time_step;
            vstore4(vector, 0, (buffer+index+1 + (*counter)*(*gsz)));
            *counter = (*counter) + 1;
        }
        return;
}
         ...
         __global uint *exit_c, __local bool *ok, __local bool *arrange)
{
    ...
    int index_vec = (gud_0*lsz_0 + lid_0)*5;  
    ...
    for (int i = 0; i <= dimension; i++){
        ...
        to_global_mem_array(buffer_print, val, &tiempo_f, lid_0*5, &dim_print, &counter);
        tiempo_i = tiempo_f;

//        for(int j=0; j<5; j++) local_buffer[lid_0 + j] = 0.0;
